...
void GreenMonkey_LinkedList::deleteNode(int val)
{
	...
	GreenMonkey* found = head;
	GreenMonkey* prev = head;

	while (found->get_next() != NULL)
	{
		if (found->get_next()->get_ID() == val)
		{
			prev = found->get_next();
...
void Habitat::showGrid()
{
	...
	for (x = 0; x < RowCol;x++)
	{
		...
		for(y=0; y < RowCol;y++)
		{
			if (grid[x][y].get_Head() != NULL)
			{
				cout << "[" << grid[x][y].get_Head()->get_gender() << setw(5)<< "]"<< endl;
				continue;
			}//endif
			cout << left << "[ "<<setw(5)<< "] ";
		}//endfor
	...
	for (x = 0; x < RowCol;x++)
	{
		for (y = 0; y < RowCol;y++)
		{
			if (grid[x][y].get_Head() != NULL)
			{
				...
				cout << left << setw(15)<< "Name" << setw(5)<< "Age" << setw(5) << "Weight" << setw(5) << "Gender"
				<< setw(5) << "Status" << "Genes" << setw(5) << "Injured"<<endl;
